...
void updateVerts()
{
    ...
    for (int scanIndex = 0; scanIndex<541; scanIndex++) {
        ...
        #define y_brakezone 30 //default 61
    ...
    // sendMessage  = speed;
    char speedChar[21];
    sprintf(speedChar,"%d",(int)(speed*10));
    ...
    int percent_speed = 100*speed/velocity_cm;
    printf("epoch: %d, scanNumber: %s, totalBytesRead: %d, stop: %d, closest_y: %d, closest_reading:%d angle: %f speed: %f percent_speed: %d \n", (int)time(0), scanNumber, totalBytesRead, stop, closest_y_cm, closest_meas, angle, speed, percent_speed);


    char output[256] = "";
    sprintf(output,"speed: %f, percent_speed: %d\n", speed, percent_speed);
